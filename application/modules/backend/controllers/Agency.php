...
class Agency extends MY_Controller{
  ...
  function index()
  {
    $this->template->set_title("Agency");
    $this->template->view("content/agency/index");
  }
    ...
    if ($this->input->is_ajax_request()) {
      ...
      foreach ($list as $rows) {
          ...
          $row[] = '<span class="data-person"><i class="ti-id-badge"></i> '.$rows->nik.'</span>
                    <span class="data-person"><i class="ti-user"></i> '.$rows->nama.' [ <span class="text-success">'.$rows->username.'</span> ]</span>
                    <span class="data-person"><i class="ti-email"></i> '.$rows->email.'</span>
                    ';
          $row[] = "Rp.".format_rupiah(balance($rows->id_person));
